...
class FullScreen:
    ...
    def __init__(self):        
        ...
        self.img = np.memmap(self.filename_img, dtype=np.uint8, mode="w+", shape=self.shape)
        
        self.tmpdir_flag = tempfile.TemporaryDirectory()
        self.filename_flag = os.path.join(self.tmpdir_flag.name, "tmp_flag.dat")
        self.flag = np.memmap(self.filename_flag, dtype=np.uint8, mode="w+", shape=(1))
    ...
    def _launch_fullscreen_app(self):
        class QWidgetFullScreen(QtWidgets.QLabel):
            def __init__(self, filename_img, shape, filename_flag):
                ...
                self.img = np.memmap(filename_img, 
                                    dtype=np.uint8, 
                                    mode="r", 
                                    shape=shape)
                ...
                self.flag = np.memmap(filename_flag, 
                                    dtype=np.uint8, 
                                    mode="r+", 
                                    shape=(1))
            ...
            def update_image(self):
                if self.flag:
                    qt_img = QtGui.QImage(self.img.flatten(), 
                                        self.width, 
                                        self.height, 
        ...
        app = QtWidgets.QApplication([])
        fullscreen_widget = QWidgetFullScreen(self.filename_img, self.shape, self.filename_flag)
        sys.exit(app.exec_())
